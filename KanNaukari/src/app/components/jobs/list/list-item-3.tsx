...
const ListItemThree = ({ item }: { item: IJobType }) => {
  const { joB_ID, job_to_date, job_salary,  city, job_experience,job_title } = item || {};
        ...
        <div className="d-sm-flex align-items-center justify-content-between mt-auto">
          <div className="d-flex align-items-center">
            ...
            <Link href={`/job-details-v1/${joB_ID}`}
              className={`job-duration fw-500 ${job_to_date === "Part time" ? "part-time" : ""}`}
            >
              {job_to_date}
            </Link>
          </div>
