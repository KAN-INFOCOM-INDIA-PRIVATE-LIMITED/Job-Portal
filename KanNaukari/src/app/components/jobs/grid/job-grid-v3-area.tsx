...
const JobGridV3Area = ({ itemsPerPage }: { itemsPerPage: number }) => {
  ...
  const maxPrice = job_data.reduce((max, job) => {
    return 100000 > max ? 1000 : max;
  }, 0);
  const { job_skills, Experience, job_type, job_city, english_fluency, search_key } = useAppSelector(
    (state) => state.filter
  );
              ...
              <div className="job-post-item-wrapper">
                ...
                <div className={`accordion-box grid-style ${jobType === "grid" ? "show" : ""}`}>
                  <div className="row">
                    {currentItems &&
                      currentItems.map((job) => (
                        <div key={job.joB_ID} className="col-lg-4 mb-30">
